...
//            SettingsCell(type: .toggle, title: "Label NSFW Sources", target: "Browse.labelNsfwSources"),
    ...
    func confirmAction(title: String, message: String, continueActionName: String = "Continue", destructive: Bool = true, proceed: @escaping () -> Void) {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let target = cells[indexPath.section][indexPath.row].target {
            ...
            case "Advanced.clearChapterCache":
                confirmAction(title: "Clear Chapter Cache", message: "This action is irreversible. Cached chapters will be cleared and redownloaded.") {
            ...
            case "Advanced.clearMangaCache":
                confirmAction(title: "Clear Manga Cache", message: "This action is irreversible. Cached Manga not in your library will be removed.") {
                    DataManager.shared.purgeManga()
                }
            case "Advanced.clearNetworkCache":
                confirmAction(title: "Clear Network Cache", message: "This action is irreversible. Cached network requests and images will be cleared.") {
            ...
            case "Advanced.reset":
                confirmAction(title: "Reset", message: "This action is irreversible. All data, settings, and caches will be cleared and reset.") {
                    KingfisherManager.shared.cache.clearMemoryCache()
                    KingfisherManager.shared.cache.clearDiskCache()
                    KingfisherManager.shared.cache.cleanExpiredDiskCache()
