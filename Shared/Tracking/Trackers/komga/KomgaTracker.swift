...
class KomgaTracker: EnhancedTracker {
        ...
        if let highestChapterRead {
            ...
            if split.count == 3 {
                ...
                if state?.lastReadVolume == nil || state?.lastReadVolume == 0 {
                    try await api.update(sourceKey: split[0], url: split[1], update: .init(lastReadVolume: Int(floor(highestChapterRead))), mangaId: split[2])
                }
    ...
    func search(for manga: Manga, includeNsfw: Bool) async throws -> [TrackSearchItem] {
        ...
        if state != nil {
            return [.init(id: "\(manga.sourceId)\(idSeparator)\(apiUrl)\(idSeparator)\(manga.id)", trackerId: id, tracked: true)]
        } else {
            return []
        }
    }
