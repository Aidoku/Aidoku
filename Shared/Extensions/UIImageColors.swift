...
extension UIImage {
    var sortedColorComparator: Comparator {
        { main, other -> ComparisonResult in
            if let m = main as? UIImageColorsCounter, let o = other as? UIImageColorsCounter {
                if m.count < o.count {
    ...
    func scaleDownSize(quality: UIImageColorsQuality) -> CGSize {
        if quality != .highest {
            if self.size.width < self.size.height {
                let ratio = self.size.height/self.size.width
                return CGSize(width: quality.rawValue/ratio, height: quality.rawValue)
            } else {
                let ratio = self.size.width/self.size.height
                return CGSize(width: quality.rawValue, height: quality.rawValue/ratio)
            }
        } else {
            return self.size
        }
    ...
    public func getColors(quality: UIImageColorsQuality = .high) -> UIImageColors? {
        guard let resizedImage = self.resizeForUIImageColors(newSize: scaleDownSize(quality: quality)) else { return nil }
        ...
        let sortedColorComparator = sortedColorComparator
        ...
        if sortedColors.count > 0 {
            guard let color = sortedColors.object(at: 0) as? UIImageColorsCounter else { return nil }
        ...
        if proposedEdgeColor.color.isBlackOrWhite && sortedColors.count > 0 {
            for i in 1..<sortedColors.count {
                if let nextProposedEdgeColor = sortedColors.object(at: i) as? UIImageColorsCounter,
                   Double(nextProposedEdgeColor.count)/Double(proposedEdgeColor.count) > 0.3 {
        ...
        for color in sortedColors {
            if let color = (color as? UIImageColorsCounter)?.color {
                if proposed[1] == -1 {
